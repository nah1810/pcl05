...
(* abstract syntax and values for \timpcore 396d *)
(* definitions of [[exp]] and [[value]] for \timpcore 395d *)
datatype value = NUM   of int
               | ARRAY of value array
(* definitions of [[exp]] and [[value]] for \timpcore 395e *)
datatype exp   = LITERAL of value
               ...
               (* array extensions to \timpcore's abstract syntax 412a *)
               | AAT   of exp * exp
               | APUT  of exp * exp * exp
               | AMAKE of exp * exp
               | ASIZE of exp
(* definition of type [[func]], to represent a \timpcore\ function 396c *)
datatype func = USERDEF   of name list * exp
              | PRIMITIVE of value list -> value
(* definition of [[def]] for \timpcore 396a *)
type userfun = { formals : (name * ty) list, body : exp, returns : ty }
datatype def = VAL    of name * exp
             | EXP    of exp
             | DEFINE of name * userfun
(* definition of [[unit_test]] for \timpcore 396b *)
...
(* definition of [[valueString]] for \timpcore 1111a *)
...
(* definition of [[expString]] for \timpcore 1110a *)
fun expString e =
  let fun bracket s = "(" ^ s ^ ")"
...
(* definitions of [[defString]] and [[defName]] for \timpcore 1110b *)
fun defString d =
  let fun bracket s = "(" ^ s ^ ")"
      val bracketSpace = bracket o spaceSep
...
(* abstract syntax and values for \timpcore 412b *)
...
(* utility functions on \timpcore\ values 1107 *)
...
(* type checking for \timpcore 402a *)
fun typeof (e, globals, functions, formals) =
  let
 (* function [[ty]], to check the type of an expression, given $\itenvs$ 402b *)
      fun ty (LITERAL v) = INTTY

 (* function [[ty]], to check the type of an expression, given $\itenvs$ 403a *)
        | ty (VAR x) = (find (x, formals) handle NotFound _ => find (x, globals)
                                                                               )

 (* function [[ty]], to check the type of an expression, given $\itenvs$ 403b *)
        | ty (SET (x, e)) =
 ...
 (* function [[ty]], to check the type of an expression, given $\itenvs$ 404a *)
        | ty (IFX (e1, e2, e3)) =
            let val tau1 = ty e1
                val tau2 = ty e2
                val tau3 = ty e3
 ...
 (* function [[ty]], to check the type of an expression, given $\itenvs$ 404b *)
        | ty (WHILEX (e1, e2)) =
            let val tau1 = ty e1
                val tau2 = ty e2
            in  if eqType (tau1, BOOLTY) then
 ...
 (* function [[ty]], to check the type of an expression, given $\itenvs$ 404c *)
        | ty (BEGIN es) =
             let val bodytypes = map ty es
             in  List.last bodytypes handle Empty => UNITTY
             end

 (* function [[ty]], to check the type of an expression, given $\itenvs$ 405a *)
        | ty (EQ (e1, e2)) =
 ...
 (* function [[ty]], to check the type of an expression, given $\itenvs$ 405b *)
        | ty (PRINTLN e) = (ty e; UNITTY)
        | ty (PRINT   e) = (ty e; UNITTY)

 (* function [[ty]], to check the type of an expression, given $\itenvs$ 405c *)
        | ty (APPLY (f, actuals)) =
             let val actualtypes                     = map ty actuals
                 val FUNTY (formaltypes, resulttype) = find (f, functions)
                 (* definition of [[parameterError]] 406a *)
...
(* type checking for \timpcore 406b *)
fun elabdef (d, globals, functions) =
    case d
      of (* cases for elaborating definitions in \timpcore 406c *)
           VAL (x, e) =>
         ...
         (* cases for elaborating definitions in \timpcore 407a *)
         | EXP e =>
             let val tau = typeof (e, globals, functions, emptyEnv)
             in  (globals, functions, typeString tau)
             end
         (* cases for elaborating definitions in \timpcore 407b *)
...
(* lexical analysis and parsing for \timpcore, providing [[filexdefs]] and [[stringsxdefs]] 1111b *)
...
(* parsers and [[xdef]] streams for \timpcore 1112a *)
...
(* parsers and [[xdef]] streams for \timpcore 1113a *)
...
(* parsers and [[xdef]] streams for \timpcore 1113b *)
...
(* parsers and [[xdef]] streams for \timpcore 1113c *)
...
(* parsers and [[xdef]] streams for \timpcore 1113d *)
val xdeftable = usageParsers
  [ ("(use filename)", USE <$> name)
  (* rows added to \timpcore\ [[xdeftable]] in exercises 1113f *)
...
val xdef =  DEF  <$> deftable 
        ...
        <|> DEF <$> EXP <$> exp
        <?> "definition"
(* parsers and [[xdef]] streams for \timpcore 1113e *)
...
(* evaluation, testing, and the read-eval-print loop for \timpcore 1114a *)
(* definitions of [[eval]] and [[evaldef]] for \timpcore 1114b *)
val unitVal = NUM 1983
(* definitions of [[eval]] and [[evaldef]] for \timpcore 1115a *)
...
fun eval (e, globals, functions, formals) =
  ...
  let val toBool = bool
      ...
      fun ev (LITERAL n)          = n
        ...
        | ev (APPLY (f, args)) =
            (case find (f, functions)
               of PRIMITIVE p  => p (map ev args)
                | USERDEF func =>
                    (* apply user-defined function [[func]] to [[args]] 1115b *)
        ...
        (* more alternatives for [[ev]] for \timpcore 412c *)
        | ev (AAT (a, i)) =
...
(* definitions of [[eval]] and [[evaldef]] for \timpcore 1116a *)
fun evaldef (d, globals, functions) =
  case d 
    of VAL (x, e) => (* evaluate [[e]] and bind the result to [[x]] 1116b *)
...
val _ = op evaldef : def * value ref env * func env -> value ref env * func env
                                                                        * string
(* definitions of [[basis]] and [[processDef]] for \timpcore 408a *)
...
(* definition of [[testIsGood]] for \timpcore 1108a *)
        ...
        | checkTypeErrorPasses d =
            (let val t = deftystring d
             in  failtest ["check-type-error failed: expected ", defString d,
                      ...
                      | Located (_, TypeError _) => true)
      (* definition of [[checkFunctionTypePasses]] 1108b *)
...
(* implementations of \timpcore\ primitives and definition of [[initialBasis]] 409a *)
...
(* utility functions and types for making \timpcore\ primitives 1116e *)
val arithtype = FUNTY ([INTTY, INTTY], INTTY)
(* type declarations for consistency checking *)
val _ = op arithtype : funty
(* utility functions and types for making \timpcore\ primitives 1117a *)
...
val initialBasis =
  let fun addPrim ((name, prim, funty), (tfuns, vfuns)) = 
      ...
      val (tfuns, vfuns)  = foldl addPrim (emptyEnv, emptyEnv)
                            (
                           (* primitive functions for \timpcore\ [[::]] 1116f *)
                             ("+", arithOp op +,   arithtype) :: 
                             ("-", arithOp op -,   arithtype) :: 
                             ("*", arithOp op *,   arithtype) :: 
                             ("/", arithOp op div, arithtype) ::

                           (* primitive functions for \timpcore\ [[::]] 1116g *)
                           ...
                           (* primitive functions for \timpcore\ [[::]] 1117b *)
